...
metadata, and shipping them to Datadog (`us5.datadoghq.com`). This is a
**Datadog-only** setup (no Oodle dual-write).
...
Datadog Agent (agent: one per host, publishes host 8126/tcp, 8125/udp)
   ├─ APM traces ──▶ Datadog (us5)   [service:rails-demo, env:sales]
   ├─ DogStatsD   ──▶ Datadog (us5)
   └─ host/process/container metrics ▶ Datadog (us5)
```
...
- `convox.yml` — the agent manifest (image, `agent:` host ports, privileged, volumes, DD env).
- `.env` / `.env.example` — `DD_API_KEY` + `DD_SITE` (secrets; injected via `convox env`, never committed).
