...
This guide documents the changes made to enable dual-write functionality, sending logs to both local Elasticsearch and Oodle simultaneously.
...
1. Create a `.env` file in the `elasticsearch-alternative/dual-write` directory:
   ```bash
   cp .env.example .env
   ```
...
1. **Check local Elasticsearch** (should have logs):
   ```bash
...
service:
  pipelines:
    logs:
      receivers: [otlp]
      processors: [batch]
      exporters: [elasticsearch, otlphttp/oodle]  # Both exporters
```
...
### No logs in local Elasticsearch
