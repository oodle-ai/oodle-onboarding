...
LLM observability demo using [official OpenTelemetry GenAI instrumentation](https://github.com/open-telemetry/opentelemetry-python-contrib/tree/main/instrumentation-genai/opentelemetry-instrumentation-google-genai) to auto-instrument Google Gemini calls and export traces and logs to Oodle.

Unlike the [traceloop-demo](../traceloop-demo) which uses a third-party SDK, this demo uses **only first-party OpenTelemetry packages** for LLM observability.
...
|---------|-------------|------|
| app | Flask app with OTel GenAI instrumentation + Gemini | 8090 |
...
| Aspect | traceloop-demo | llmops-otel-demo |
|--------|---------------|------------------|
| Instrumentation | Traceloop SDK (third-party) | Official OTel GenAI instrumentation (first-party) |
| Setup | `Traceloop.init()` + `@workflow` decorators | `GoogleGenAiSdkInstrumentor().instrument()` + explicit OTel SDK |
| Content capture | Automatic | Opt-in via `OTEL_SEMCONV_STABILITY_OPT_IN` + `OTEL_INSTRUMENTATION_GENAI_CAPTURE_MESSAGE_CONTENT` |
| Export config | `TRACELOOP_BASE_URL` env var | Standard `OTEL_EXPORTER_OTLP_ENDPOINT` env var |
| Signals | Traces | Traces + logs (prompt/response as span attributes and log events) |
| Semantic conventions | OpenLLMetry | Official OTel GenAI semantic conventions (`gen_ai.*`) |
